...
function jobHandlers.gemDps(payload)
	...
	for _, gemId in ipairs(payload.gemIds) do
		...
		if gemData then
			local okCalc, output = pcall(skillsTab.CalcGemSwapOutput, skillsTab, group, payload.gemIndex, gemData, calcFunc, useFullDPS, nil)
...
function jobHandlers.itemPower(payload)
	local statEntry = data.powerStatList[payload.statIndex]
	if not statEntry then
		...
		return { workerError = string.format("itemPower: no data.powerStatList entry at index %s",
			tostring(payload.statIndex)) }
	end
	local calcFunc = build.calcsTab:GetMiscCalculator()
	local useFullDPS = statEntry.stat == "FullDPS"
	local results = { }
...
function jobHandlers.nodePower(payload)
	...
	local function extract(output)
		...
		-- The whole numeric minion table, not just CombinedDPS: GetFromOutput
		-- reads output.Minion[X] for Minion-prefixed power stats and adds the
		-- minion side to every damage stat, and a vector missing those fields
		-- silently zeroes the minion contribution
		if output.Minion then
			local minion = { }
			for stat, value in pairs(output.Minion) do
