...
function calcs.perform(env, skipEHP)
	...
	for _, activeSkill in ipairs(env.player.activeSkillList) do
		activeSkill.skillModList = newModList(activeSkill.baseSkillModList)
			...
			if isThisSpectreActive then
				...
				for modId = 1, #spectreData.modList do
					...
					elseif modData.name == "AllyModifier" and modData.type == "LIST" then
						buffs["Spectre"] = buffs["Spectre"] or newModList()
						minionBuffs["Spectre"] = minionBuffs["Spectre"] or newModList()
			...
			elseif buff.type == "Warcry" then
				if env.mode_buffs then
					...
					if not activeSkill.skillModList:Flag(nil, "CannotShareWarcryBuffs") then
						...
						local extraWarcryModList = activeSkill.activeEffect.grantedEffect.name == "Rallying Cry" and newModList() or {}
			...
			elseif buff.type == "Curse" or buff.type == "CurseBuff" then
				...
				if (not skipCurse) and (env.mode_effective and (not enemyDB:Flag(nil, "Hexproof") or modDB:Flag(nil, "CursesIgnoreHexproof") or activeSkill.skillData.ignoreHexLimit or activeSkill.skillData.ignoreHexproof) or mark) then
					...
					if buff.type == "Curse" then
						...
						local temp = newModList()
						temp:ScaleAddList(buff.modList, mult)
						curse.buffModList = newModList()
						...
						if env.minion then
							curse.minionBuffModList = newModList()
	...
	buffExports["Aura"]["extraAura"] = { effectMult = 1, modList = newModList() }
