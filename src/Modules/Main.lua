...
function main:OpenOptionsPopup(savedState)
	...
	local function workerPoolRamLabel()
		local count = self.workerPoolCount or 16
		-- ~300 MB measured per worker: each holds a full copy of the program data
		-- and the current build in its own Lua state
		return count == 0 and "^8no worker RAM" or string.format("^8~%.1f GB RAM when active", count * 0.3)
	end
	...
	nextRow()
	controls.mcpServerEnabled = new("CheckBoxControl"):CheckBoxControl({ "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 20 }, "^7Allow external build control (MCP):", function(state)
	...
	controls.mcpServerPort = new("EditControl"):EditControl({ "LEFT", controls.mcpServerEnabled, "RIGHT" }, { 90, 0, 70, 18 }, tostring(self.mcpServerPort or 49080), nil, "%D", 5, function(buf)
		self.mcpServerPort = m_min(m_max(m_floor(tonumber(buf) or 49080), 1024), 65535)
	end)
	controls.mcpServerPortLabel = new("LabelControl"):LabelControl({ "RIGHT", controls.mcpServerPort, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, "^7Port:")
	...
	controls.mcpServerStatus = new("LabelControl"):LabelControl({ "LEFT", controls.mcpServerPort, "RIGHT" }, { 8, 0, 0, 16 }, function()
